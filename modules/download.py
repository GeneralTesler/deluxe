...
def getextfrommime(buf):
    ...
    if mf == 'application/msword':
        return 'doc'
    elif mf == 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
        return 'docx'
        ...
        return 'ppt'
    elif mf == 'application/vnd.openxmlformats-officedocument.presentationml.presentation':
        return 'pptx'
...
def downloadlist(urls,outdir):
    ...
    if len(dltargets) > 0:
        ...
        for url in dltargets:
            ...
            r = requests.get(url,headers=ua,stream=True,allow_redirects=True,verify=False)
            try:
